...
document.addEventListener('contextProvided', () => {
    ...
    if (context.myEvent.sessions.length === 0) {
        document.getElementById('rowTarget').innerHTML = '<tr><td colspan="5" class="bx--table-no-results"><i>No sessions found.</i></td></tr>';
    } else {
        ...
        <tr class="bx--expandable-row bx--expandable-row--hidden" data-child-row>
            ...
            const workshopRows = session.workshops.map((workshop) => {
                const rowString = `<div class="bx--structured-list-row ${workshop.name === 'Tutoring with a Teacher' || workshop.name === 'Teacher Tutoring' ? 'teacherTutor' : ''}">
                <div class="bx--structured-list-td bx--structured-list-content--nowrap">
                    <a href="EventWorkshop.html?workshop_id=${workshop.id}&session_id=${session.id}">${workshop.name}</a></div>
